...
//! O(1) zero-copy pub/sub over shared memory.
//!
//! Allocates independent buffers from a shared pool and transfers only an
//! 8-byte descriptor (block index + data length) regardless of payload size.
//!
//! The subscriber gets a **safe** zero-copy reference ([`SampleGuard`]) that
//! holds the block alive via atomic refcounting.
//!
...
// ─── Region (shared between publisher and subscriber) ─────────────────
...
impl Region {
    ...
    #[allow(clippy::cast_possible_truncation)]
    fn check_heartbeat(&self) -> Result<(), IpcError> {
...
/// Handle returned by [`ShmPublisher::register`]. Identifies a topic
/// for use with [`ShmPublisher::loan`].
#[derive(Clone)]
...
/// [`SampleGuard`] references — no `unsafe` needed to read data.
///
/// # Examples
///
/// ```rust,no_run
