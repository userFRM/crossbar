...
//!
//! fn get_user(req: Request) -> String {
...
///
/// The router stores its routes behind an [`Arc`] so that cloning it for
/// repeated dispatch contexts is O(1).
///
...
impl Router {
    ...
    /// no route matches.
    ///
    /// This method is used internally by [`InProcessClient`](crate::inproc::InProcessClient)
    /// and is also available publicly so callers can dispatch requests
    /// directly without going through a transport wrapper.
    ///
    /// # Examples
    ///
    /// ```rust
